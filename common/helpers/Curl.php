...
class Curl
{
    ...
    static function http_curl($url,$is_post = false,$data = array(),$type = 'json',$res = 'json'){
        $model = new Curl();
        if($is_post){
            $function = '';
            switch ($type) {
                case 'query':
                $model->setOption(CURLOPT_POSTFIELDS, http_build_query($data));
                break;
                case 'json':
                $model->setOption(CURLOPT_POSTFIELDS, Json::encode($data));
                break;
                case 'default':
                $model->setOption(CURLOPT_POSTFIELDS, $data);
                break;
            }
            $response = $model
    ...
    static function http_curl_ssl($url,$xml){
         $model = new Curl();
         $model->setOption(CURLOPT_POSTFIELDS, $xml);
         $model->setOption(CURLOPT_SSL_VERIFYPEER, false);
         $model->setOption(CURLOPT_SSL_VERIFYHOST,false);
         $model->setOption(CURLOPT_SSLCERT,getcwd().'/cert/apiclient_cert.pem');
         $model->setOption(CURLOPT_SSLKEY,getcwd().'/cert/apiclient_key.pem');
         $model->setOption(CURLOPT_CAINFO,'/cert/rootca.pem');
         $response = $model->post($url);
         return $response;
    }
