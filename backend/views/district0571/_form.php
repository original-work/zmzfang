...
<div class="district-form">
    ...
    <div><input type="button" onclick="getAutoLoc($('#district<?=$cityl?>-districtname').val())" value="自动获取定位(通过小区名)">
    <input type="button" onclick="getAutoLoc($('#district<?=$cityl?>-address').val())" value="自动获取定位(通过地址)">
    ...
    <div class="form-group field-district-region">
        ...
        <select class="form-control" name="District<?=$cityl?>[plate]" id="district-plate">
...
<script src="http://www.zmzfang.com/weixin/js/area.js"></script>
 ...
 <script type="text/javascript">
    ...
    $('#district<?=$cityl?>-region').on("change",function(){
        initOption($(this).val());
    });
  ...
  map.addControl(geolocationControl);
    ...
    function getLoc(e){
        ...
        if($('#allowClick').prop("checked")){
            ...
            $('#district<?=$cityl?>-longitude').val(e.point.lng);
            $('#district<?=$cityl?>-latitude').val(e.point.lat);
        }
    }
    function getAutoLoc(loc){
        ...
        myGeo.getPoint(loc, function(point){
            if (point) {
                ...
                map.addOverlay(marker);               // 将标注添加到地图中
                marker.setAnimation(BMAP_ANIMATION_BOUNCE); //跳动的动画
                $('#district<?=$cityl?>-longitude').val(point.lng);
                $('#district<?=$cityl?>-latitude').val(point.lat);
            }else{
                alert("没有解析到结果!");
            }
        }, "<?=$city?>市");
    ...
    function setPlace(){
        ...
        function myFun(){
            ...
            map.addOverlay(marker);               // 将标注添加到地图中
            marker.setAnimation(BMAP_ANIMATION_BOUNCE); //跳动的动画
            $('#district<?=$cityl?>-longitude').val(pp.lng);
            $('#district<?=$cityl?>-latitude').val(pp.lat);
        }
